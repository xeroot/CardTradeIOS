...
class RulesTableViewController: UITableViewController {
        ...
        Alamofire.request("http://192.168.1.2:45455/api/Rules").responseJSON{
            ...
            if let json = response.result.value{
                ...
                for(_,subJson):(String,JSON) in sJson{
                    let objRules = Rules()
                    objRules.Description = subJson["Description"].stringValue
                    objRules.type = subJson["type"].stringValue
                    self.myRules.append(objRules)
                }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let result = "Type: " + myRules[indexPath.row].type
        let detail = "Description: " + myRules[indexPath.row].Description
        // Configure the cell...
